...
// 1. Updated Icons
import { Bot, User, Send, Sparkles, Brain, Loader2, LogOut, SearchCheck } from 'lucide-react'; 
...
const CHAT_API_ENDPOINT = '/api/chat/pattern-decoder';
const HISTORY_API_ENDPOINT = '/api/chat/history/pattern-decoder';

export default function PatternDecoderPage() {
    ...
    <div className="flex flex-col h-screen bg-slate-950 text-slate-100">
      ...
      <header className="flex items-center justify-between px-6 py-4 bg-slate-900 border-b border-slate-800">
        <div className="flex items-center gap-3">
          <SearchCheck className="w-6 h-6 text-purple-400" /> {/* New Icon */}
          <div>
            <h1 className="text-lg font-semibold">Business Pattern Decoder</h1>
            <div className="flex items-center gap-2 text-xs text-purple-400"> {/* New Color */}
              <span className="flex h-2 w-2 relative"><span className="animate-ping absolute h-full w-full rounded-full bg-purple-400 opacity-75"></span><span className="relative inline-flex rounded-full h-2 w-2 bg-purple-500"></span></span>
              Llama 3.1 70B (Fast Analysis)
            </div>
          </div>
        </div>
        <button
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          {!isLoadingHistory && messages.length === 0 && (
            <div className="text-center mt-20 space-y-4">
              <Sparkles className="w-10 h-10 mx-auto text-purple-500 opacity-50" />
              <h2 className="text-2xl font-bold">Welcome!</h2>
              <p className="text-slate-400">Let's find the #1 bottleneck holding your business back.</p>
            </div>
          )}

          {/* 7. Message Loop (Updated Icon) */}
          {messages.map((m) => (
            <div key={m.id} className={`flex gap-3 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              {m.role !== 'user' && (
                <div className="w-9 h-9 rounded-full bg-slate-800 flex items-center justify-center flex-shrink-0 border border-slate-700">
                  <SearchCheck className="w-5 h-5 text-purple-400" />
                </div>
              )}
          ...
          {isLoading && (
            <div className="flex justify-start gap-3">
              <div className="w-9 h-9 rounded-full bg-slate-800 flex items-center justify-center flex-shrink-0 border border-slate-700"><SearchCheck className="w-5 h-5 text-purple-400" /></div>
